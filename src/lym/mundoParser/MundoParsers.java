...
import lym.languages.SwiftViews;
...
public class MundoParsers {
	...
	*/
	...
	public SwiftViews getParserSwiftViews(){
		return new SwiftViews(System.in);
	}
	...
	public String  procesarCadena(String texto) {
		...
		*/
		if(parsers.get(currentParser).equals("Parser SwiftViews")){
			SwiftViews parserSwiftViews = getParserSwiftViews();
			parserSwiftViews.ReInit(new java.io.StringReader(texto));
			try {
		    	parserSwiftViews.one_line();
		    	resp = new String("OK    \n");
		    }catch (Exception e) {
		        resp = new String ("Error de Sintaxis: "+e.getMessage());
		     } catch (Error e) {
